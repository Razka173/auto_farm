...
gold_target = 250000
elixir_target = 250000
dark_elixir_target = 500

gold_remaining = 50000
...
def Bot():
    ...
    while bot_running.is_set():
        ...
        while (
                (
                    int(f.check_loot.result[0]) < gold_target and gold_target_enabled == True
                )
                or (
                    int(f.check_loot.result[1]) < elixir_target and elixir_target_enabled == True
                )
                or (
                    int(f.check_loot.result[2]) < dark_elixir_target and dark_elixir_target_enabled == True
                )
...
class CoCBotApp:
    ...
    def get_targets(self):
        return {
            "gold_target": int(self.gold_target_entry.get()),
            "elixir_target": int(self.elixir_target_entry.get()),
            "dark_elixir_target": int(self.dark_elixir_target_entry.get()),
            "gold_remaining": int(self.gold_remaining_entry.get()),
            "elixir_remaining": int(self.elixir_remaining_entry.get()),
            "dark_elixir_remaining": int(self.dark_elixir_remaining_entry.get()),
            ...
            "troop_slots": int(self.troop_slots_entry.get()),
            "heroes_slots": int(self.heroes_slots_entry.get()),
            "spell_slots": int(self.spell_slots_entry.get()),
        }
